	...
	for x=X, X+W do
		for y=Y, Y+H do
			if GetMapType(x,y) == Tile.BLOCK then
				SetMap(x,y,1,  Tile.PASS)-- tile or  1
...
function Carve(wall, side, roomId)
	...
	if corridor == 1 then expansions = {{0,2},{1,3}}
	...
	for i = 0, steps do
		if #expansions == 0 then break end
		...
		if CheckRoom(rx+xd,ry+yd,rw+wd,rh+hd,roomId) then
			...
			--table.remove(expansions, indexOfOne(side))
		end
	end
	...
	if(rw==1 or rh == 1) then corridor=1 end
...
function Dig()
	...
	if digs == 0 then
		...
		if not wall then stuck = stuck+1 
			...
			if not side then stuck = stuck+1 
				...
				MakeRoom(wall[1],wall[2],1,1, TILE_DOOR, roomId) --Door
				...
				if math.random() < 0.95 then --Branch
					for i=0, 2 do
						local wall = PickWall(room.x, room.y, room.w, room.h)
	...
	if(tilesDug < (w*h)*percentToFill and stuck < tooMuchStuck) then
		...
		local highestId = 0
		for i=1, #rooms do
			if rooms[i].gen > highest and not rooms[i].corridor then
				highest=rooms[i].gen
				highestId = i
			end
		end
